...
const labels = computed(() => {
  if (props.kind === 'equipment') {
    return {
      ...
      conditionsText: 'Регулярное ТО и резервные машины — работаем без срывов сроков. Доставляем технику на объект собственным транспортом из Владимира.'
    }
  }
  ...
  return {
    ...
    conditionsText: 'Доставляем материал самосвалами 20 тонн. Подбираем подходящую машину под объём заказа и условия подъезда к объекту.'
  }
})
...
const conditionsList = computed(() => {
  ...
  if (props.kind === 'service') {
    return [
      { main: 'Заявка', sub: 'обсудим объём, сроки и адрес объекта' },
      { main: 'Расчёт и договор', sub: 'выезд на объект, смета, договор за 1 день' },
      { main: 'Предоплата', sub: 'резерв техники и материалов по договору' },
      { main: 'Выполнение работ', sub: 'контроль качества и отчёты заказчику' },
      { main: 'Приёмка работ', sub: 'акт выполненных работ, КС-2 и КС-3' },
      { main: 'Оплата', sub: 'окончательный расчёт после приёмки, с НДС и без' }
    ]
  }
  return [
    ...
    <div class="wrap">
      ...
      <div v-if="kind === 'equipment' && data.pricing" class="content-block">
        ...
        <p style="margin-top: 20px; font-size: 13px;">
          Стоимость указана с учётом услуг оператора, ГСМ и доставки по Владимиру и области.
